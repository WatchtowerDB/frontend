...
import type { AssertionItem, CheckStatus } from "@/types/compliance"
...
function toLiveAssertionStatus(assertion: AssertionItem): "passed" | "failed" | "pending" {
  if (assertion.status === "COMPLETED") {
    return assertion.result ? "passed" : "failed"
  }
  return "pending"
}
...
export function useComplianceStreams() {
  ...
  const removeLiveAssertionsByCheckId = useComplianceCheckStore(
    (s) => s.removeLiveAssertionsByCheckId,
  )
  ...
  useEffect(() => {
    if (liveChecksData === undefined) return
    const liveCheckIdsFromBackend = new Set(liveChecksData?.results.map((c) => c.id) ?? [])
    ...
    for (const id of runningIds) {
      if (!liveCheckIdsFromBackend.has(id)) {
    ...
    for (const checkId of liveCheckIdsFromBackend) {
      if (!liveCheckIdsFromBackend.has(checkId)) {
        if (controllersRef.current[checkId]) {
          controllersRef.current[checkId].abort()
      ...
      ;(async () => {
        try {
          const { results: assertions } = await getAssertions({ check: [checkId] })
          for (const assertion of assertions) {
            upsertLiveAssertion(assertion.id, checkId, {
              status: toLiveAssertionStatus(assertion),
              streamingDone: assertion.status === "COMPLETED" || assertion.status === "FAILED",
            })
          }
        } catch (e) {
          ...
          `${import.meta.env.VITE_BACKEND_URL}/api/compliance/checks/${checkId}/stream/`,
          {
            get headers() {
              ...
              if (lastEventId) {
                ...
                // This baby now handles if (it was already generating, or it just started generating),
                // simultaneously while no lastEventId is present in localStorage.
            ...
            onmessage(e) {
              ...
              if (type.endsWith("phase.update")) {
                ...
                if (step === "execution" && status === "started")
                  setCheckPhase(checkId, "executing")
